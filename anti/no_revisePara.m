...
function [v_lambda_new, D_cell] = no_revisePara(error_quan_col, lambda, tau)
...
%             [x,fval,exitflag]=fmincon('obj_fun',lambda,[],[],[],[],lambda*0.1,lambda*10,'con_fun',opts, lambda,pre, prepre,boundary_next_p, quantized_step, v_N(i), N, alpha1, alpha2);
%             fprintf('need to revise at pos %d, ori = %f, revised = %f\n', v_q(i), v_lambda1(i), x);
%              if (lambda*0.05> x | x> lambda*10 )
...
%   clear lambda_pre;
%      %%%%%handle the  case where qk >0
%     for i = length(v_lambda1)-1:-1:zero_indx+1    % lambda of bin 0 has already be revised by the case of negtive. length(v_lambda1) is 1 bigger than that of v_boundary_pre(-5.5)
%        cur_q        =  v_q(i);
